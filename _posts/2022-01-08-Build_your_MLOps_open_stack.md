...
When we run the final scripts using DVC/Airflow/Prefect in the Dev/Prod environment, the batch predictions can be persisted as a table in the database. I have used the BQ table to save the batch predictions. The model artifacts can be pushed to GCS bucket for versioning and automation purpose.
